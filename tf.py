...
with open(file, 'rb') as handler:
    train_labels = pickle.load(handler)
...
sample_analysis.display(train_images[0])
print(train_y_hot[0])
...
W_conv1 = weight_variable([1, 1, 1, 200])
...
# display 100 fetures in 10 by 10 grid
layer1 = tf.reshape(h_conv1, (-1, image_height, image_width, 20, 10))
# reorder so the channels are in the first dimension, x and y follow.
layer1 = tf.transpose(layer1, (0, 3, 1, 4,2))
layer1 = tf.reshape(layer1, (-1, image_height*10, image_width*20))

W_conv2 = weight_variable([2, 2, 200, 64])
...
cross_entropy = -tf.reduce_sum(y_*tf.log(y_conv)) #tf.reduce_mean(-tf.reduce_sum(y_ * tf.log(y_conv), reduction_indices=[1]))
