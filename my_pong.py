...
returns = discount_rewards(np.array([1, 0, 1]))
print(returns)
...
while True:
    ...
    time_step += 1
    print('time_step:', time_step, 'reward is ', reward, 'done=', done)
    ...
    if done:  #
        ...
        unique_rewards, reward_counts = np.unique(epr, return_counts=True)
        for (r, c) in enumerate(unique_rewards, reward_counts):
            print('epi', episode_number, ': reward', r, 'happened ', c, 'times')
